...
**v0.7.0 port note:** the Sail v0.7.0 port (`feat/0.7.0`, port-v0.6.6 Phase 7) wires the same
`compression-codec` option + `write.parquet.compression-codec` table property into
`WriterConfig.writer_properties`, but defaults to **zstd** — the Iceberg spec's documented
default for `write.parquet.compression-codec` — instead of v0.6.6's Snappy. This keeps the
option override (option layer wins, then table property, then the zstd default) identical to
the design below; only the fallback default differs (and the parquet crate's own default is
`UNCOMPRESSED`, so the zstd default is a strict improvement either way).
