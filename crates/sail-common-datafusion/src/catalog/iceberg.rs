...
/// Mirrors Apache Iceberg's `MetadataTableType`. Only the tables needed are modeled
/// today; `from_name` returns `None` for unknown names so it can be used as a cheap
