...
struct ClientRegistry {
    ...
    /// Client ids that explicitly released their session, with the release
    /// time. A released client no longer counts as live; if it reconnects,
    /// `touch` clears the mark. Timestamps let `sweep` bound this set so
    /// client churn cannot grow it without limit.
...
impl ClientRegistry {
    ...
    fn sweep(&mut self, now: Instant) {
        ...
        self.released
            .retain(|_, released_at| now.duration_since(*released_at) < CLIENT_REGISTRY_TTL);
        // `released` ids carry no liveness weight (their entries are gone), so
        // if churn still overflows the cap, drop the oldest release marks.
        // This only forgets "already gone" bookkeeping, never live clients.
        while self.released.len() > CLIENT_REGISTRY_MAX_ENTRIES {
    ...
    fn release_client(&mut self, client_id: &str) -> bool {
        ...
        self.released.insert(client_id.to_string(), Instant::now());
        // Bound the set even between sweeps: release bursts without intervening
        // `touch` calls must not grow memory without limit.
        if self.released.len() > CLIENT_REGISTRY_MAX_ENTRIES {
            self.sweep(Instant::now());
        }
        present
    }
    ...
    fn has_live_clients(&self) -> bool {
        self.entries
            .keys()
            .any(|client_id| !self.released.contains_key(client_id))
    }
}
...
impl MultiplexedSparkConnectServer {
    ...
    fn stamp(
        ...
        match self.registry.lock() {
            ...
            // but an untracked client is invisible to release bookkeeping, so make
            // the fallback observable instead of silent.
            ...
            Err(_) => {
                warn!("multiplexer client registry lock poisoned; request proceeds untracked");
                Ok(std::mem::replace(
                    session_id_field,
...
mod tests {
    ...
    fn registry_touch_and_release() {
        ...
        assert!(registry.release_client("a"));
        assert!(!registry.entries.contains_key("a"));
        assert!(registry.released.contains_key("a"));
    }
    ...
    fn registry_sweep_evicts_only_stale_entries() {
        ...
        registry.released.insert("stale-release".to_string(), stale);
        registry.released.insert("fresh-release".to_string(), fresh);
        registry.sweep(now);
        assert!(!registry.entries.contains_key("stale"));
        assert!(registry.entries.contains_key("fresh"));
        assert!(!registry.released.contains_key("stale-release"));
        assert!(registry.released.contains_key("fresh-release"));
    }

    #[test]
    fn registry_release_marks_stay_bounded_under_churn() {
        let mut registry = ClientRegistry::default();
        // Release bursts with no intervening touches (e.g. unknown ids) must
        // not grow memory without limit.
        for i in 0..CLIENT_REGISTRY_MAX_ENTRIES + 10 {
            registry.release_client(&format!("gone-{i}"));
        }
        assert!(registry.released.len() <= CLIENT_REGISTRY_MAX_ENTRIES);
