...
impl MultiplexedSparkConnectServer {
    ...
    pub fn new(session_manager: SessionManager, canonical_session_id: Option<String>) -> Self {
        ...
        Self {
            ...
            session_manager,
            canonical_session_id,
            registry: Mutex::new(ClientRegistry::default()),
        }
    }
    ...
    /// Registers the client id and swaps the field in place to the canonical
    /// id, returning the original client id for the reverse direction.
    fn stamp(&self, session_id_field: &mut String) -> String {
        match self.registry.lock() {
            // Bookkeeping is best-effort: a poisoned lock must never fail requests.
            Ok(mut registry) => {
                stamp_session_id(&mut registry, &self.canonical_session_id, session_id_field)
            }
            Err(_) => std::mem::replace(session_id_field, self.canonical_session_id.clone()),
        }
    }
}
...
impl SparkConnectService for MultiplexedSparkConnectServer {
    ...
    async fn execute_plan(
        ...
        debug!("{request:?}");
        let client_id = self.stamp(&mut request.session_id);
        let response = self
            .inner
    ...
    async fn add_artifacts(
        ...
        debug!("{first:?}");
        let mut first = first;
        let client_id = self.stamp(&mut first.session_id);
    ...
    /// entry goes away; the backend is never called.
