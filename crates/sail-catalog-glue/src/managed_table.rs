...
pub(crate) fn apply_alter_table_options(
    ...
    match options {
        ...
        AlterTableOptions::AlterColumnComment { .. } => {
            return Err(CatalogError::NotSupported(
                "AWS Glue catalog does not support ALTER COLUMN COMMENT".to_string(),
            ));
        }
        AlterTableOptions::AlterColumnNullability { .. } => {
            return Err(CatalogError::NotSupported(
                "AWS Glue catalog does not support ALTER COLUMN NULLABILITY".to_string(),
            ));
        }
        AlterTableOptions::AlterColumnPosition { .. } => {
            return Err(CatalogError::NotSupported(
                "AWS Glue catalog does not support ALTER COLUMN POSITION".to_string(),
            ));
        }
        ...
        AlterTableOptions::RenameTable { .. } => {
            return Err(CatalogError::NotSupported(
                "RENAME TABLE is not yet supported for Glue catalog".to_string(),
            ));
        }
        AlterTableOptions::AddColumns { .. } => {
            return Err(CatalogError::NotSupported(
                "ADD COLUMNS is not yet supported for Glue catalog".to_string(),
            ));
        }
        AlterTableOptions::DropColumns { .. } => {
            return Err(CatalogError::NotSupported(
                "DROP COLUMNS is not yet supported for Glue catalog".to_string(),
            ));
        }
    }
    Ok(parameters)
