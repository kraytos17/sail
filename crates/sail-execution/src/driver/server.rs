...
use crate::driver::gen_::driver_service_server::DriverService;
...
use crate::driver::{DriverEvent, gen_};
...
impl DriverService for DriverServer {
    ...
    async fn report_task_status(
        ...
        let status = gen_::TaskStatus::try_from(status).map_err(ExecutionError::from)?;
        let cause = cause
