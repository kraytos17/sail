...
/// A read-only [`TableProvider`] that exposes an Iceberg table's metadata as data
/// (e.g. `db.table.refs` / `db.table.snapshots`).
///
/// Mirrors Apache Iceberg's metadata tables (`RefsTable`, `SnapshotsTable`): each row
/// is materialized in memory from the table's `TableMetadata`, and `WHERE` / `ORDER BY`
/// / `LIMIT` are applied by DataFusion above this provider. Metadata tables are tiny,
/// so no pushdown is needed.
#[derive(Debug, Clone)]
pub struct IcebergMetadataTableProvider {
    /// The table location (URI) of the base table.
    table_uri: String,
    /// The base table metadata the rows are materialized from.
    metadata: TableMetadata,
    /// Which metadata table to expose.
    metadata_type: IcebergMetadataTableType,
    /// The fixed Arrow schema for `metadata_type`.
    schema: SchemaRef,
}
...
impl IcebergMetadataTableProvider {
    ...
    /// Materialize all rows for this metadata table into a single Arrow batch.
...
impl TableProvider for IcebergMetadataTableProvider {
    ...
    fn supports_filters_pushdown(
        ...
        // Metadata tables are tiny; let the planner apply predicates above the scan.
        Ok(vec![
...
/// Build the `summary` map column for the snapshots metadata table. Each snapshot's
/// summary is a map from string to string; the operation is included as a key.
...
mod tests {
    ...
    /// Executes a logical plan against a `SessionContext` with the given metadata provider
    /// registered as `test_tbl`, returning every cell as a `ScalarValue` (row-major).
    fn run_plan(
        ...
        tokio::runtime::Runtime::new()
            ...
            .block_on(async move {
                ...
                ctx.register_table("test_tbl", std::sync::Arc::new(provider))
                    .unwrap();
                ...
                for batch in &batches {
                    for row in 0..batch.num_rows() {
                        for col in batch.columns() {
                            out.push(
                                datafusion::common::ScalarValue::try_from_array(col, row).unwrap(),
                            );
                        }
                    }
                }
    ...
    #[test]
    fn refs_supports_heimdall_current_snapshot_query() {
        let provider = IcebergMetadataTableProvider::new(
            "s3://bucket/tbl",
            sample_metadata(),
            IcebergMetadataTableType::Refs,
        );
        // heimdall: GetCurrentIcebergSnapshotID
        // SELECT CAST(snapshot_id AS STRING) FROM test_tbl.refs WHERE name = 'main'
        let plan = LogicalPlanBuilder::scan(
            "test_tbl",
            datafusion::datasource::provider_as_source(std::sync::Arc::new(provider.clone())),
            None,
        )
        .unwrap()
        .filter(col("name").eq(lit("main")))
        .unwrap()
        .project(vec![
        ...
        .build()
        .unwrap();
        let row = run_plan(provider, plan);
        assert_eq!(row.len(), 1);
        assert_eq!(row[0].to_string(), "2");
    }

    #[test]
    fn snapshots_supports_heimdall_latest_snapshot_query() {
        let provider = IcebergMetadataTableProvider::new(
            "s3://bucket/tbl",
            sample_metadata(),
            IcebergMetadataTableType::Snapshots,
        );
        // heimdall: GetLatestIcebergSnapshotID
        // SELECT CAST(snapshot_id AS STRING) FROM test_tbl.snapshots
        ...
        .unwrap()
        .sort(vec![col("committed_at").sort(false, false)])
        .unwrap()
        .limit(0, Some(1))
        .unwrap()
        .project(vec![
    ...
    #[test]
    fn snapshots_supports_heimdall_snapshot_exists_and_parent_queries() {
        // heimdall: CheckSnapshotExists
        // SELECT 1 FROM test_tbl.snapshots WHERE snapshot_id = 2
        let provider = IcebergMetadataTableProvider::new(
            "s3://bucket/tbl",
            sample_metadata(),
            IcebergMetadataTableType::Snapshots,
        ...
        .unwrap()
        .filter(col("snapshot_id").eq(lit(2i64)))
        .unwrap()
        .project(vec![lit(1i64).alias("1")])
        .unwrap()
        .build()
        .unwrap();
        let row = run_plan(provider.clone(), plan);
        assert_eq!(row.len(), 1);

        // heimdall: GetSnapshotParent
        // SELECT CAST(parent_id AS STRING) FROM test_tbl.snapshots WHERE snapshot_id = 2
        let plan = LogicalPlanBuilder::scan(
            "test_tbl",
            datafusion::datasource::provider_as_source(std::sync::Arc::new(provider.clone())),
            None,
