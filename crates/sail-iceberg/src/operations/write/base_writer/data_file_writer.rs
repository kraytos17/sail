...
/// Read parquet column statistics from footer metadata, keyed by the parquet-embedded
/// field IDs. External parquet files carry no Iceberg field IDs, so callers that need to
/// produce Iceberg-keyed stats should use `aggregate_from_parquet_metadata_with_field_map`.
pub(crate) fn aggregate_from_parquet_metadata(
