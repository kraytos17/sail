...
impl ArrowParquetWriter {
    ...
    /// Total encoded bytes buffered so far: bytes already flushed to the sink plus the
    /// current in-progress row group.
    ///
    /// This is what writers use to roll a new data file once a target file size is reached,
    /// mirroring DataFusion's `ArrowWriter` convention and the DeltaLake writer's
    /// `buffer.len() + in_progress_size()` estimate.
    ///
    /// Synchronous so it can be checked without holding the (non-`Sync`) parquet writer across
    /// an await; the flushed-byte count is tracked by the buffer via an atomic counter.
    ...
    pub async fn close(mut self) -> Result<(Bytes, ParquetFileMeta), String> {
        ...
        // `close` consumes the writer, dropping its clone of the shared buffer so that
        // `into_inner` below can `Arc::try_unwrap` and hand back the full byte vector.
        let metadata = writer
            .close()
            .await
...
/// Build parquet `WriterProperties` for Iceberg data file writes, honoring the standard
/// Iceberg `write.parquet.*` table properties on top of the resolved compression codec.
///
/// Only recognized keys are consumed; unrecognized keys are ignored (forward compatible).
/// When a key is absent, the parquet crate's built-in defaults apply (e.g. statistics on),
/// so current behavior is preserved while exposing opt-out / tuning knobs. This is the single
/// place data-file write properties are derived, keeping writer behavior consistent across
/// LOAD and INSERT paths.
pub(crate) fn build_writer_properties(
    ...
    // Look up a table property by (case-insensitive) key.
    ...
    // Table-wide statistics opt-out: write.metadata.metrics.default = none|counts|full.
    ...
    // Per-column tuning: write.parquet.* keys that the parquet writer understands.
