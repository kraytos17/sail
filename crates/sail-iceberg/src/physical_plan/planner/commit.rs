...
pub async fn assemble_iceberg_commit_plan(
    ...
    // Reserved for a rewrite-with-remove path (union of the writer output with a plan
    // that removes data files); all current callers pass `None`.
    ...
    Ok(Arc::new(IcebergCommitExec::new_with_reported_row_count(
