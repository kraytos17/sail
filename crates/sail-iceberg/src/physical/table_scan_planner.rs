...
impl ExtensionPlanner for IcebergPhysicalPlanner {
    async fn plan_extension(
        ...
        if let Some(load_data) = node.as_any().downcast_ref::<LoadDataNode>() {
            if load_data.target_format().eq_ignore_ascii_case("iceberg") {
                return plan_load_data(session_state, load_data).await.map(Some);
            }
        }
