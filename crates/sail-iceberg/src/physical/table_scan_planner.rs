...
use sail_logical_plan::call_procedure::CallProcedureNode;
...
use crate::physical::call_procedure_planner::plan_call_procedure;
...
impl ExtensionPlanner for IcebergPhysicalPlanner {
    async fn plan_extension(
        ...
        if let Some(call_node) = node.as_any().downcast_ref::<CallProcedureNode>() {
            return plan_call_procedure(session_state, call_node)
                .await
                .map(Some);
        }
